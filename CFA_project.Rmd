...
```{r}
# converting discounting values that have text responses to NAs
new_df <- df %>%
  mutate(
    response = ifelse(str_detect(response, "[^0-9.]"), NA, response),
    affected_subjects = ifelse(str_detect(response, "[^0-9.]"), subject_id, NA)
  )
```
